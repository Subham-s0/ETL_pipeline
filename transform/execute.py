...
def load_and_clean(spark,input_dir,output_dir):
    """ Stage1 :load data ,drop duplicates, remove nulls ,save cleaned data. """ 
    #define Schemas
    ...
    recommendations_schema = T.StructType([
        T.StructField("id",T.StringType(),False),
        T.StructField("related_ids",T.ArrayType(T.StringType()),False),
    ])
...
def create_query_tables(output_dir, artists_df, recommendations_df, tracks_df):
    logger.info("Stage 3: Create query-optimized tables.")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        logger.warning("Usage: python script.py <input_dir> <output_dir>")
        sys.exit(1)
